...
def plot_prm(planner, title="PRM (10000 samples, 100 obstacles)"):
    ...
    for n in planner.nodes:
        ...
        if hasattr(n, "x"):
            ...
            ys.append(n[1])
    if xs:
        ax.scatter(xs, ys, s=5, color="blue", label="Nodes")
    ...
    elif hasattr(planner, "adj"):
        for i, nbrs in enumerate(planner.adj):
            for j, _ in nbrs:
                ni = planner.nodes[i]
                nj = planner.nodes[j]
                x1 = ni.x if hasattr(ni, "x") else ni[0]
...
if __name__ == "__main__":
    ...
    print("Running PRM with 10000 samples...")
    prm = PRMPlanner(start=start,goal=goal,bounds=bounds,num_samples=10000,k_neighbors=10,)
    prm.set_obstacles(obstacles)

    success = prm.plan()
    print("Success:", success)
    print("Planning time:", prm.get_planning_time())
    print("Number of nodes:", prm.get_num_nodes())
